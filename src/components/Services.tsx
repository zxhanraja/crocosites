...
export default function Services() {
  ...
  return (
    <section id="services" ref={containerRef} className="py-40 px-6 md:px-8 bg-background border-t border-border">
          ...
          {services.map((svc, i) => (
            ...
            >
              ...
              <div className="flex gap-8 items-start min-h-[120px]">
                {/* Vertical Title */}
                <h3 className="text-lg md:text-3xl font-black text-foreground uppercase tracking-tighter origin-top-left rotate-90 whitespace-nowrap translate-x-[75%] md:translate-x-full pt-2 shrink-0">
                  {svc.title}
                </h3>

                {/* Description - Swaps to Funny on Hover */}
                <div className="relative flex-1 pl-8 md:pl-10">
                  ...
                  <p className={`absolute top-0 left-8 md:left-10 text-accent font-black leading-[1.3] text-lg md:text-xl transition-all duration-500 ${hoveredIndex === i ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2'}`}>
                    {svc.funny}
                  </p>
                </div>
              </div>
